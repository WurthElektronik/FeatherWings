...
bool atecc608a_init() {
    ...
    if (status == ATCA_SUCCESS) {
        WE_DEBUG_PRINT("ATECC608A init done\r\n");
        return true;
    } else {
        WE_DEBUG_PRINT("ATECC608A init 0X%02X\r\n", status);
        return false;
    }
}
...
bool atecc608a_test_enc_dec() {
    ...
    for (int idx = 0; idx < 4; idx++) {
        if (!atecc608a_generate_random(plaintext + (idx * 32))) {
            WE_DEBUG_PRINT("Random data generation fail\r\n");
            return false;
        }
    }
    WE_DEBUG_PRINT("Plain text : ");
    for (int i = 0; i < 128; i++) {
        WE_DEBUG_PRINT("%02X", plaintext[i]);
    }
    WE_DEBUG_PRINT("\r\n");

    // Encrypt data
    if (!atecc608a_encrypt_data(ATCA_TEMPKEY_KEYID, plaintext, 128, cipherText,
                                authTag, NULL, iv)) {
        WE_DEBUG_PRINT("Encryption failed\r\n");
        return false;
    }

    WE_DEBUG_PRINT("IV : ");
    for (int i = 0; i < (AES_IV_LENGTH); i++) {
        WE_DEBUG_PRINT("%02X", iv[i]);
    }
    WE_DEBUG_PRINT("\r\n");
    WE_DEBUG_PRINT("Cipher text : ");
    for (int i = 0; i < 128; i++) {
        WE_DEBUG_PRINT("%02X", cipherText[i]);
    }
    WE_DEBUG_PRINT("\r\n");

    WE_DEBUG_PRINT("Auth tag : ");
    for (int i = 0; i < AES_AUTH_TAG_SIZE; i++) {
        WE_DEBUG_PRINT("%02X", authTag[i]);
    }
    WE_DEBUG_PRINT("\r\n");

    // Decrypt data
    if (!atecc608a_decrypt_data(ATCA_TEMPKEY_KEYID, cipherText, 128, iv,
                                authTag, decryptedText, &isVerified)) {
        WE_DEBUG_PRINT("Decryption failed \r\n");
        return false;
    }

    WE_DEBUG_PRINT("Decrypted data : ");
    for (int i = 0; i < 128; i++) {
        WE_DEBUG_PRINT("%02X", decryptedText[i]);
    }
    WE_DEBUG_PRINT("\r\n");

    if (isVerified == false) {
        WE_DEBUG_PRINT("Decryption authentication failed \r\n");
    } else {
        WE_DEBUG_PRINT("Decrypted data authenticated\r\n");
    }

    // Check if the plain text and decrypted texts match
    if (memcmp(plaintext, decryptedText, 128) == 0) {
        WE_DEBUG_PRINT("Decrypted text matches plain text\r\n");
        return true;
    }
    return false;
}
...
bool atecc608a_test_echd() {
    ...
    if (atecc608a_gen_ephemeral_key(ECDH_KEY_SLOT, ephemeral_Pub_alice)) {
        WE_DEBUG_PRINT("Ephemeral Public key Alice : ");
        for (int i = 0; i < 64; i++) {
            WE_DEBUG_PRINT("%02X", ephemeral_Pub_alice[i]);
        }
        WE_DEBUG_PRINT("\r\n");
    }
    ...
    if (atecc608a_gen_ephemeral_key(ECDH_KEY_SLOT + 1, ephemeral_Pub_bob)) {
        WE_DEBUG_PRINT("Ephemeral Public key Bob : ");
        for (int i = 0; i < 64; i++) {
            WE_DEBUG_PRINT("%02X", ephemeral_Pub_bob[i]);
        }
        WE_DEBUG_PRINT("\r\n");
    }

    // Derive shared secret using the private key in slot 2 and input public key
    // corresponding to slot 3
    if (atecc608a_derive_shared_secret(ECDH_KEY_SLOT, ephemeral_Pub_bob,
                                       shared_alice, true)) {
        WE_DEBUG_PRINT("Shared secret Alice: ");
        for (int i = 0; i < 32; i++) {
            WE_DEBUG_PRINT("%02X", shared_alice[i]);
        }
        WE_DEBUG_PRINT("\r\n");
    }

    // Derive shared secret using the private key in slot 3 and input public key
    // corresponding to slot 2
    if (atecc608a_derive_shared_secret(ECDH_KEY_SLOT + 1, ephemeral_Pub_alice,
                                       shared_bob, true)) {
        WE_DEBUG_PRINT("Shared secret Bob: ");
        for (int i = 0; i < 32; i++) {
            WE_DEBUG_PRINT("%02X", shared_bob[i]);
        }
        WE_DEBUG_PRINT("\r\n");
    }
    ...
    if (memcmp(shared_alice, shared_bob, 32) == 0) {
        WE_DEBUG_PRINT("Derived secrets match\r\n");
    } else {
        return false;
    }
    ...
    if (atecc608a_derive_symmetric_key(salt, info, aesKey, true)) {
        WE_DEBUG_PRINT("AES key: ");
        for (int i = 0; i < 16; i++) {
            WE_DEBUG_PRINT("%02X", aesKey[i]);
        }
        WE_DEBUG_PRINT("\r\n");
    }
    return true;
...
bool atecc608a_test_sign_verify() {
    ...
    if (atecc608a_read_device_public_key(publicKey)) {
        WE_DEBUG_PRINT("Public key: ");
        for (int i = 0; i < 64; i++) {
            WE_DEBUG_PRINT("%02X", publicKey[i]);
        }
        WE_DEBUG_PRINT("\r\n");
    }

    // Input data, signature and the public key to verify the signature
    if (atecc608a_verify_signature(rnd, signature, publicKey,
                                   &signatureVerified)) {
        if (signatureVerified == true) {
            WE_DEBUG_PRINT("Sign verified\r\n");
        } else {
            WE_DEBUG_PRINT("Sign verification failed\r\n");
        }
    }
 ...
 *                   If false, the shared secret is written to the tempKey
 * register(in SRAM) When stored in tempKey, the sharedSecret can be used for
 * further operations (ex. KDF)
 * @retval true if successful false in case of failure
 */
bool atecc608a_derive_shared_secret(uint8_t keySlot, uint8_t *publicKey,
                                    uint8_t *sharedSecret, bool showClear) {
    uint8_t status;
    if (showClear == true) {
 ...
 * @param  showClear If true, return the shared secret
 *                   If false, the shared secret is written to the destination
 * key slot When stored in keyslot, the AES key can be used for further
 * operations (ex. encryption/decryption)
 * @retval true if successful false in case of failure
 */
bool atecc608a_derive_symmetric_key(uint8_t *salt, uint8_t *info,
                                    uint8_t *aesKey, bool showClear) {
    uint8_t status;
    uint8_t mode;
    uint16_t keyID = 0;
 ...
 * @retval true if successful false in case of failure
 */
bool atecc608a_generate_SHA256_digest(uint8_t *data, int dataLength,
                                      uint8_t *digest) {
    uint8_t status;
...
/**
 * @brief  Verify if the data was signed by the private key corresponding to the
 * input public key
 ...
 * @retval true if successful false in case of failure
 */
bool atecc608a_verify_signature(uint8_t *message, const uint8_t *signature,
                                const uint8_t *publicKey, bool *isVerified) {
    uint8_t status;
 ...
 * @param  dataLenth Length of data to encrypt (Multiple of block length =
 * 16bytes)
 ...
 * @retval true if successful false in case of failure
 */
bool atecc608a_encrypt_data(uint32_t keySlot, uint8_t *plainText,
                            uint16_t dataLength, uint8_t *cipherText,
                            uint8_t *authTag, uint8_t *iv_fixed, uint8_t *iv) {
    uint8_t status;
    ...
    status = atcab_aes_gcm_init_rand(&ctx, keySlot, 0, AES_IV_RND_SIZE,
                                     iv_fixed, AES_IV_FIXED_SIZE, iv);
    if (status != ATCA_SUCCESS) {
        return false;
    }
    ...
    for (int idx = 0; idx < numberOfBlocks; idx++) {
        status = atcab_aes_gcm_encrypt_update(
            &ctx, (plainText + (idx * AES_BLOCK_LENGTH)), AES_BLOCK_LENGTH,
            (cipherText + (idx * AES_BLOCK_LENGTH)));
        if (status != ATCA_SUCCESS) {
            return false;
        }
    }
 ...
 * @retval true if successful false in case of failure
 */
bool atecc608a_decrypt_data(uint32_t keySlot, uint8_t *cipherText,
                            uint16_t dataLength, uint8_t *iv, uint8_t *authTag,
                            uint8_t *plaintext, bool *isVerified) {
    uint8_t status;
    atca_aes_gcm_ctx_t ctx;
    ...
    // Encrypt the data blockwise
    for (int idx = 0; idx < numberOfBlocks; idx++) {
        status = atcab_aes_gcm_decrypt_update(
            &ctx, (cipherText + (idx * AES_BLOCK_LENGTH)), AES_BLOCK_LENGTH,
            (plaintext + (idx * AES_BLOCK_LENGTH)));
        if (status != ATCA_SUCCESS) {
            return false;
        }
    }
    status = atcab_aes_gcm_decrypt_finish(&ctx, authTag, AES_AUTH_TAG_SIZE,
                                          isVerified);
    if (status != ATCA_SUCCESS) {
        return false;
    }
...
void atecc608a_print_certs() {
    ...
    do {
        ...
        // Use the end of the sector buffer to temporarily hold the data to save
        // RAM
        file_list = ((uint8_t *)sector_buffer) +
                    (sizeof(sector_buffer) - TLS_FILE_NAME_MAX * 2);
        ...
        if (atca_status != ATCACERT_E_SUCCESS) {
            WE_DEBUG_PRINT("Certificates not found %i\r\n", atca_status);
            // Break the do/while loop
            break;
        }
        pem_cert_size = sizeof(pem_cert);
        atcacert_encode_pem_cert(signer_cert, signer_cert_size, pem_cert,
                                 &pem_cert_size);
        WE_DEBUG_PRINT("Signer Cert : \r\n%s\r\n", pem_cert);
        // Get the signer's public key from its certificate
        atca_status = atcacert_get_subj_public_key(
            &g_tngtls_cert_def_1_signer, signer_cert, signer_cert_size,
            signer_public_key);
        if (atca_status != ATCACERT_E_SUCCESS) {
            WE_DEBUG_PRINT("atcacert_get_subj_public_key failed 0X%02X\r\n",
                           atca_status);
            // Break the do/while loop
            break;
        }
        ...
        atca_status =
            atcacert_read_cert(&g_tngtls_cert_def_3_device, signer_public_key,
                               device_cert, &device_cert_size);
        if (atca_status != ATCACERT_E_SUCCESS) {
            WE_DEBUG_PRINT("atcacert_read_cert failed 0X%02X\r\n", atca_status);
            // Break the do/while loop
            break;
        }
        pem_cert_size = sizeof(pem_cert);
        atcacert_encode_pem_cert(device_cert, device_cert_size, pem_cert,
                                 &pem_cert_size);
        WE_DEBUG_PRINT("Device Cert : \r\n%s\r\n", pem_cert);

        atca_status =
            atcacert_get_subj_key_id(&g_tngtls_cert_def_3_device, device_cert,
                                     device_cert_size, subject_key_id);
        if (atca_status != ATCACERT_E_SUCCESS) {
            WE_DEBUG_PRINT("atcacert_get_subj_key_id failed 0X%02X\r\n",
                           atca_status);
            // Break the do/while loop
            break;
        }
        ...
        atca_status =
            atcacert_get_cert_sn(&g_tngtls_cert_def_3_device, device_cert,
                                 device_cert_size, cert_sn, &cert_sn_size);
        if (atca_status != ATCACERT_E_SUCCESS) {
            WE_DEBUG_PRINT("atcacert_get_cert_sn failed 0X%02X\r\n",
                           atca_status);
            // Break the do/while loop
            break;
        }
        ...
        WE_DEBUG_PRINT("Device cert serial number : %s \r\n",
                       device_cert_filename);

        device_cert =
            NULL;  // Make sure we don't use this now that it has moved

        // Get the signer certificate SN for the filename
        cert_sn_size = sizeof(cert_sn);
        atca_status =
            atcacert_get_cert_sn(&g_tngtls_cert_def_1_signer, signer_cert,
        ...
        WE_DEBUG_PRINT("Signer cert serial number : %s \r\n",
...
static const char *bin2hex(const void *data, size_t data_size) {
    ...
    if (data_size * 2 > sizeof(buf) - 1) return "[buf too small]";
 ...
 * @retval true if successful false in case of failure
 */
bool atecc608a_hex2bin(const char *hex, size_t hex_size, uint8_t *bin,
                       size_t *bin_size) {
    if (hex_size % 2 == 1) {
        return false;
    }
